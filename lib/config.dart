import 'package:flutter/material.dart';

//MAIN MENU

double menuButtonHeight = 50.0;
double menuButtonWidth = 200.0;
double menuButtonFontSize = 25;
Color menuButtonColor = Colors.lightBlue;
Color menuButtonHighlightColor = Colors.blue;
double intervalBetweenMenuButtons = 15;

//NEW GAME
double paddingFromTop = 20;
double fontSize = 15;
double buttonHeight = 40;
double buttonWidth = 40;
double keyboardButtonHeight=45;
double keyboardButtonWidth=45;
double borderWidth = 2;
double intervalBetweenText = 10;
double intervalBetweenButtons = 10;
int delayBeforeShow = 400;
int delayToMemorize = 500;
int delayBetweenlevels = 1600;
int delayWhileChecking = 50;



